...
import com.sojpt.model.BigData;
...
public class BigDataAdminController extends IViewController {


    @RequestMapping("list")
    public void list() {

        BigData bigData = getModel(BigData.class, "bigData");

        SqlPara sqlPara = Db.getSqlPara("admin_data.select", getKv(bigData));
    ...
    public void delete() {
        BigData bigData = getModel(BigData.class, "bigData");
        if(BigData.dao.deleteById(bigData.getId())) {
            renderJson(Ret.ok());
        } else {
            renderJson(Ret.fail());
        }
    ...
    public void batchDelete() {
        ...
        SqlPara sqlPara = Db.getSqlPara("admin_data.deleteByIds", Kv.by("idList", Arrays.asList(ids)));
