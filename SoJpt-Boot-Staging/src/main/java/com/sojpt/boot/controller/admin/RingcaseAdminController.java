...
import com.sojpt.model.Ringcase;
 ...
 */
@RestController
@RequestMapping("/admin/ringcase/")
@Before({AdminLoginInterceptor.class, AdminPopedomInterceptor.class})
public class RingcaseAdminController extends IViewController {

    @RequestMapping("list")
    public void list() {

        Ringcase ringcase = getModel(Ringcase.class, "ringcase");

        SqlPara sqlPara = Db.getSqlPara("admin_ringcase.select", getKv(ringcase));
    ...
    public void delete() {
        Ringcase ringcase = getModel(Ringcase.class, "ringcase");
        if(Ringcase.dao.deleteById(ringcase.getId())) {
            renderJson(Ret.ok());
        } else {
            renderJson(Ret.fail());
        }
    ...
    public void batchDelete() {
        ...
        SqlPara sqlPara = Db.getSqlPara("admin_ringcase.deleteByIds", Kv.by("idList", Arrays.asList(ids)));
