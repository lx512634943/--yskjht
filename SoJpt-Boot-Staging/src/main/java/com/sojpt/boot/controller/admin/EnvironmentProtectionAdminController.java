...
import com.sojpt.model.EnvironmentProtection;
 ...
 */
@RestController
@RequestMapping("/admin/protection/")
@Before({AdminLoginInterceptor.class, AdminPopedomInterceptor.class})
public class EnvironmentProtectionAdminController extends IViewController {
    ...
    public void list() {
        ...
        EnvironmentProtection environmentProtection = getModel(EnvironmentProtection.class, "environmentProtection");

        SqlPara sqlPara = Db.getSqlPara("admin_protection.select", getKv(environmentProtection));
    ...
    public void update() {
        EnvironmentProtection environmentProtection = getModel(EnvironmentProtection.class, "environmentProtection");
        if(environmentProtection.update()) {
            renderJson(Ret.ok());
        } else {
            renderJson(Ret.fail());
        }
    ...
    public void delete() {
        EnvironmentProtection environmentProtection = getModel(EnvironmentProtection.class, "environmentProtection");
        if(EnvironmentProtection.dao.deleteById(environmentProtection.getId())) {
            renderJson(Ret.ok());
        } else {
            renderJson(Ret.fail());
        }
    ...
    public void batchDelete() {
        ...
        SqlPara sqlPara = Db.getSqlPara("admin_protection.deleteByIds", Kv.by("idList", Arrays.asList(ids)));
