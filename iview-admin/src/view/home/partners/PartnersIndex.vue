  ...
  <div>
    <Card>
      ...
      <ButtonGroup style="margin-bottom: 15px;">
        ...
        <Add v-model="isOpenAddPage" @addRow="addPartnersIndex"/>
        <Edit v-model="isOpenEditPage" @updateRow="updatePartnersIndex" :partnersIndex="editPartnersIndex"/>
        <Button @click="batchDelete()">删除</Button>
      </ButtonGroup>
      <Table
        ref="tablePartnersIndex"
        :data="tableData"
        :columns="columns"
...
<script>
  import { list, del, batchDelete} from '@/api/index'
  ...
  export default {
              ...
              return h('div', [
                ...
                h(
                  'Button',
                  {
                    ...
                    on: {
                      click: () => {
                        this.editPartnersIndex = params.row
                      ...
                      click:()=>{
                        this.$Modal.confirm({
                          ...
                          onOk: () => {
                            ...
                            del({ 'partnersIndex.id': params.row.id}).then(res => {
                              this.getPartnersIndexList(this.current, this.pageSize)
                            })
                          },
    ...
    methods: {
      ...
      addPartnersIndex (row) {
        this.tableData.push(row)
      },
      updatePartnersIndex (row) {
      ...
      getPartnersIndexList (current, pageSize, paras) {
      ...
      handleRequestParams (para) {
        Object.assign(para, createFormObj(this.formPartnersIndex, 'partnersIndex'))
        return para
      },
      exportExcel () {
        this.$refs.tablePartnersIndex.exportCsv({
          filename: `table-${new Date().valueOf()}.csv`
        })
      },
      changePage (pageNumber) {
        this.getPartnersIndexList(pageNumber, this.pageSize)
      },
      changePageSize (pageSize) {
        this.getPartnersIndexList(this.current, pageSize)
      },
      remove (index) {
      ...
      onSortChange (obj) {
        if (obj.order === 'normal') {
          this.getPartnersIndexList(1, this.pageSize)
        } else {
          this.getPartnersIndexList(1, this.pageSize, {
            field: obj.key,
            order: obj.order
