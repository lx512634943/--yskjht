    ...
    >
      ...
      <FormItem label="新闻标题" :label-width="100" prop="newsid">
        <i-col span="12" style="padding-right:10px">
          <i-select v-model="formPartnersIndex.partnersid" filterable  style="width:200px" >
...
<script>
  import { save,list1 } from '@/api/index'
    ...
    methods: {
      ...
      handleSubmit (name) {
        this.$refs[name].validate(valid => {
          if (valid) {
            save(Object.assign({}, createModelObj(this.formPartnersIndex, 'partnersIndex'))).then(
              res => {
                if (res) {
