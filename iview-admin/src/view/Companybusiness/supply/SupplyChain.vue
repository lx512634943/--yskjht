  ...
  <div>
    <Card>
      <Collapse style="margin-bottom:15px;">
        <Panel>查询
          <div slot="content">
            ...
            >
              <FormItem>
                <Button type="primary" @click="handleSubmit('formSupplyChain')">查询</Button>
      ...
      <ButtonGroup style="margin-bottom: 15px;">
        ...
        <Add v-model="isOpenAddPage" @addRow="addSupplyChain"/>
        <Edit v-model="isOpenEditPage" @updateRow="updateSupplyChain" :supplyChain="editSupplyChain"/>
        <Button @click="batchDelete()">删除</Button>
      </ButtonGroup>
      <Table
        ref="tableSupplyChain"
        :data="tableData"
        :columns="columns"
...
<script>
  import { list, del, batchDelete} from '@/api/chain'
  ...
  export default {
    ...
    data () {
      return {
        ...
        columns: [
          ...
          { key: 'name', title: '供应链业务名称', sortable: false},
          { key: 'title', title: '供应链业务详情', sortable: false,tooltip:true},
          { key: 'title1', title: '供应链业务详情1', sortable: false,tooltip:true},
          { key: 'business_name', title: '公司业务名称', sortable: false},
          {
            key: 'byid_pictures', title: '供应链顶部照片', sortable: false,
            render: (h, params) => {
              return h('div', [
                h(
                  'img', {
          ...
          {
            key: 'picture', title: '供应链业务图片', sortable: false, width: 100,
            render: (h, params) => {
              return h('div', [
                h(
                  'img', {
              ...
              return h('div', [
                ...
                h(
                  'Button',
                  {
                    ...
                    on: {
                      click: () => {
                        this.editSupplyChain = params.row
                    ...
                    on: {
                      click:()=>{
                        this.$Modal.confirm({
                          ...
                          content: '<p>确定删除吗?</p>',
                          onOk: () => {
                            del({ 'supplyChain.id': params.row.id}).then(res => {
                              this.getSupplyChainList(this.current, this.pageSize)
                            })
                          },
                          onCancel: () => {}
                        });
    ...
    methods: {
      ...
      getSupplyChainList (current, pageSize, paras) {
      ...
      handleRequestParams (para) {
        Object.assign(para, createFormObj(this.formSupplyChain, 'supplyChain'))
        return para
      },
      exportExcel () {
        this.$refs.tableSupplyChain.exportCsv({
          filename: `table-${new Date().valueOf()}.csv`
        })
      },
      changePage (pageNumber) {
        this.getSupplyChainList(pageNumber, this.pageSize)
      },
      changePageSize (pageSize) {
        this.getSupplyChainList(this.current, pageSize)
      },
      remove (index) {
      ...
      onSortChange (obj) {
        if (obj.order === 'normal') {
          this.getSupplyChainList(1, this.pageSize)
        } else {
          this.getSupplyChainList(1, this.pageSize, {
            field: obj.key,
            order: obj.order
