  ...
  <div>
    <Card>
      <Collapse style="margin-bottom:15px;">
        <Panel>查询
          <div slot="content">
            ...
            >
              <FormItem>
                <Button type="primary" @click="handleSubmit('formPromotion')">查询</Button>
      ...
      <ButtonGroup style="margin-bottom: 15px;">
        ...
        <Add v-model="isOpenAddPage" @addRow="addPromotion"/>
        <Edit v-model="isOpenEditPage" @updateRow="updatePromotion" :promotion="editPromotion"/>
...
<script>
  import { list, del, batchDelete} from '@/api/promotion'
  ...
  export default {
    ...
    data () {
      return {
        ...
        columns: [
          ...
          { key: 'name', title: '信息化名称', sortable: false},
          { key: 'title', title: '信息化详情', sortable: false,tooltip:true},
          { key: 'titles', title: '信息化详情2', sortable: false,tooltip:true},
          { key: 'business_name', title: '公司业务名称', sortable: false,tooltip:true},
          {
            key: 'picture', title: '信息化详情图片', sortable: false,
            render: (h, params) => {
              return h('div', [
                h(
                  'img', {
          ...
          {
            key: 'byid_pictures', title: '信息化顶部图片', sortable: false,
            render: (h, params) => {
              return h('div', [
                h(
                  'img', {
                    ...
                    on: {
                      click:()=>{
                        this.$Modal.confirm({
                          ...
                          content: '<p>确定删除吗?</p>',
                          onOk: () => {
                            del({ 'promotion.id': params.row.id}).then(res => {
                              this.getPromotionList(this.current, this.pageSize)
                            })
                          },
                          onCancel: () => {}
                        });
    ...
    methods: {
      ...
      getPromotionList (current, pageSize, paras) {
      ...
      handleRequestParams (para) {
        Object.assign(para, createFormObj(this.formPromotion, 'promotion'))
        return para
      },
      exportExcel () {
        this.$refs.tablePromotion.exportCsv({
          filename: `table-${new Date().valueOf()}.csv`
        })
      },
      changePage (pageNumber) {
        this.getPromotionList(pageNumber, this.pageSize)
      },
      changePageSize (pageSize) {
        this.getPromotionList(this.current, pageSize)
      },
      remove (index) {
      ...
      onSortChange (obj) {
        if (obj.order === 'normal') {
          this.getPromotionList(1, this.pageSize)
        } else {
          this.getPromotionList(1, this.pageSize, {
            field: obj.key,
            order: obj.order
