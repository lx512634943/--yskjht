  ...
  >
    ...
    >
      ...
      <FormItem label="业务名称" :label-width="100" prop="business_id" >
        <Select v-model="formBusinessbyid.businessId" style="width:200px">
          <Option v-for="item in cityList" :value="item.id" :key="item.id">{{ item.name }}</Option>
        </Select>
      </FormItem>
      <FormItem label="信息化案例名字" :label-width="100" prop="buid_name">
        <Input type="text" v-model="formBusinessbyid.buid_name" placeholder="新闻标题"/>
      </FormItem>
      <FormItem label="业务详情" :label-width="100" prop="byid_title">
        <Input type="textarea" v-model="formBusinessbyid.byid_title" placeholder="新闻标题"/>
      </FormItem>
      <FormItem label="业务详情1" :label-width="100" prop="byid_titles">
        <Input type="textarea" v-model="formBusinessbyid.byid_titles" placeholder="新闻标题"/>
      </FormItem>
...
<script>
  import { save } from '@/api/businessbyid'
  ...
  export default {
    ...
    computed: {
      baseURL () {
        return this.$config.urlPath + 'admin/businessbyid/upload'
      }
    },
    methods: {
      ...
      handleSubmit (name) {
        this.$refs[name].validate(valid => {
          if (valid) {
            save(Object.assign({}, createModelObj(this.formBusinessbyid, 'businessbyid'))).then(
              res => {
                if (res) {
