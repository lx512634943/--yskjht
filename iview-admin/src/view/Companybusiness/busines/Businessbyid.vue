  ...
  <div>
    <Card>
      <Collapse style="margin-bottom:15px;">
        <Panel>查询
          <div slot="content">
            ...
            >
              <FormItem>
                <Button type="primary" @click="handleSubmit('formBusinessbyid')">查询</Button>
      ...
      <ButtonGroup style="margin-bottom: 15px;">
        ...
        <Add v-model="isOpenAddPage" @addRow="addBusinessbyid"/>
        <Edit v-model="isOpenEditPage" @updateRow="updateBusinessbyid" :businessbyid="editBusinessbyid"/>
        <Button @click="batchDelete()">删除</Button>
      </ButtonGroup>
      <Table
        ref="tableBusinessbyid"
        :data="tableData"
        :columns="columns"
...
<script>
  import { list, del, batchDelete} from '@/api/businessbyid'
  ...
  export default {
    ...
    data () {
      return {
        ...
        columns: [
          ...
          { key: 'name', title: '公司业务名称', sortable: false},
          { key: 'buid_name', title: '大健康案例名字', sortable: false},
          { key: 'byid_title', title: '大健康详情案例1', sortable: false,tooltip:true},
          { key: 'byid_titles', title: '大健康详情案例2', sortable: false,tooltip:true},
          {
          ...
          {
            key: 'picture', title: '公司业务详情图', sortable: false,
            render: (h, params) => {
              return h('div', [
                h(
                  'img', {
              ...
              return h('div', [
                ...
                h(
                  'Button',
                  {
                    ...
                    on: {
                      click: () => {
                        this.editBusinessbyid = params.row
                    ...
                    on: {
                      click:()=>{
                        this.$Modal.confirm({
                          ...
                          content: '<p>确定删除吗?</p>',
                          onOk: () => {
                            del({ 'businessbyid.id': params.row.id}).then(res => {
                              this.getBusinessbyidList(this.current, this.pageSize)
                            })
                          },
                          onCancel: () => {}
                        });
    ...
    methods: {
      ...
      getBusinessbyidList (current, pageSize, paras) {
      ...
      handleRequestParams (para) {
        Object.assign(para, createFormObj(this.formBusinessbyid, 'businessbyid'))
        return para
      },
      exportExcel () {
        this.$refs.tableBusinessbyid.exportCsv({
          filename: `table-${new Date().valueOf()}.csv`
        })
      },
      changePage (pageNumber) {
        this.getBusinessbyidList(pageNumber, this.pageSize)
      },
      changePageSize (pageSize) {
        this.getBusinessbyidList(this.current, pageSize)
      },
      remove (index) {
      ...
      onSortChange (obj) {
        if (obj.order === 'normal') {
          this.getBusinessbyidList(1, this.pageSize)
        } else {
          this.getBusinessbyidList(1, this.pageSize, {
            field: obj.key,
            order: obj.order
